...
```yaml title="k8s/postgresql.yaml"
apiVersion: helm.toolkit.fluxcd.io/v2beta1
kind: HelmRelease
metadata:
  name: postgresql
...
```yaml title="k8s/app-with-database.yaml"
apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-app
...
```yaml title="k8s/velero-namespace.yaml"
apiVersion: v1
kind: Namespace
metadata:
  name: velero
...
```yaml title="k8s/backup-schedule.yaml"
...
```yaml title="k8s/backup-monitoring.yaml"
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: velero-alerts
