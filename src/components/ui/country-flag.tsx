...
export function CountryFlag({
  ...
  if (countryCode.length === 2 && /^[A-Z]{2}$/.test(countryCode)) {
    const firstLetter = String.fromCodePoint(
      countryCode.charCodeAt(0) - 65 + 0x1f1e6,
    )
    const secondLetter = String.fromCodePoint(
      countryCode.charCodeAt(1) - 65 + 0x1f1e6,
    )
  ...
  return (
    <span
      className={cn(
        'inline-flex items-center justify-center bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded-sm px-1',
        className,
      )}
